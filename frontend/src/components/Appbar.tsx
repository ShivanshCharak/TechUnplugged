...
export const Appbar = () => {
  const {notifData} = useContext(NotificationContext);
      ...
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        >
          {["Home", "Blogs", "Pricing", "Showcase"].map((val, idx) => {
            ...
            const isActive =
              pathname === `/${route}` || (val === "Home" && pathname === "/");
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  <span
                    className={`duration-300 w-[200px] -right-[80px] absolute top-[50px] bg-[#0b0b0b] h-[100px] overflow-scroll ${
                      showNotif ? "block" : "hidden"
                    }`}
                  >
                    <ul className=" border border-zinc-900 h-full ">
                      {notifData !==undefined ? (
                        notifData.map((val, index) => {
                          return (
    <li
      key={index}
      className="flex items-center gap-3 px-4 py-3 hover:bg-black rounded-lg transition-all duration-200"
    >
      {/* Avatar */}
      <div className="w-[40px] h-[40px] flex items-center justify-center rounded-full bg-blue-800 text-white font-semibold text-sm">
        {val.user.firstname[0].toUpperCase() + val.user.lastname[0].toUpperCase()}
      </div>

      {/* Content */}
      <div className="flex flex-col">
        {/* Title */}
        <h1 className="text-sm font-medium text-white">{val.title}</h1>

        {/* Slug + Email */}
        <div className="flex flex-col">
          <p className="text-[11px] text-slate-400">{val.slug}</p>
          <p className="text-[11px] text-slate-500">{val.user.email}</p>
        </div>

        {/* User Name */}
        <h4 className="text-[12px] text-slate-300 mt-1">
          {val.user.firstname} {val.user.lastname}
                        ...
                        <li className="h-full py-auto hover:bg-black items-center justify-center w-[70%] text-sm mx-auto">
                          <MessageCircleCode className="mx-auto mt-3" />
