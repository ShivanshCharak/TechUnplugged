...
import { useState, useEffect, useContext } from 'react';
    ...
    export const useBlogData = (blog: Blog) => {
      ...
      try {
        ...
        // Convert payload to a Blob for sendBeacon
        const blob = new Blob([JSON.stringify(payload)], { type: "application/json" });

        // Send to your backend
        navigator.sendBeacon("http://localhost:8787/api/v1/blog/sync", blob);

        console.log("Data flushed to backend before unload");
      } catch (error) {
        console.error("Failed to flush data before unload:", error);
      }
    };
    useEffect(() => {
      ...
      setBlogDB(prev => {
        ...
        newDB[blog.id].reactions[type] =
          newDB[blog.id].reactions[type] === 1 ? 0 : 1;
      console.log("Setting up PostReaction ",blog.id,newDB[blog.id])

      set(`PostReactions`, JSON.stringify({[blog.id]:newDB[blog.id]}));
        return newDB;
      });
    };
      ...
      if (newComment.trim() && authData?.name) {
        ...
        try {
          const response = await fetch('http://localhost:8787/api/v1/blog/comment', {
            method: 'POST',
            credentials: 'include',
            headers: { 
